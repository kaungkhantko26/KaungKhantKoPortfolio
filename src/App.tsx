...
const fadeUp = { hidden: { opacity: 0, y: 26 }, show: { opacity: 1, y: 0, transition: { duration: 0.6, ease: EASE } } };
const inViewOnce = { once: true, margin: '-10%' } as const;

function StaggerGrid({ children, className }: { children: ReactNode; className?: string }) {
  return <motion.div className={className} variants={stagger} initial="hidden" whileInView="show" viewport={inViewOnce}>{children}</motion.div>;
}
...
function CountUp({ value, suffix = '' }: { value: number; suffix?: string }) {
  ...
  useEffect(() => {
    if (!inView || value <= 0) { setShown(value); return; }
    const controls = animate(0, value, { duration: 1.1, ease: EASE, onUpdate: v => setShown(Math.round(v)) });
    // Failsafe: always land on the real value even if the animation never runs.
    const snap = setTimeout(() => setShown(value), 1600);
    return () => { controls.stop(); clearTimeout(snap); };
  }, [inView, value]);
...
function SiteHeader({ light = false }: { light?: boolean }) {
  ...
  const isActive = (path: RoutePath) => path === '/' ? active === '/' : active === path || active.startsWith(`${path}/`);
  return <>
    <header className={`site-header ${light ? 'light' : ''}`}>
      <button className="brand" onClick={() => go('/')} aria-label="Kaung Khant Ko portfolio home"><img src="/logo.png" alt="" width={28} height={28} className="brand-mark" />KAUNG KHANT KO / PORTFOLIO</button>
      <nav>{items.map(([label, path]) => <button key={path} className={isActive(path) ? 'active' : ''} onClick={() => go(path)}>{label}</button>)}</nav>
...
function Landing() {
  return <div className="landing page"><IntroWipe /><SiteHeader light /><div className="dot-grid" />
    <main className="landing-main">
      ...
      <motion.div className="route-picker" variants={stagger} initial="hidden" whileInView="show" viewport={inViewOnce}><RouteCard number="01" title="GRAPHIC DESIGN" subtitle="Brand identity · UI/UX · Artwork" path="/design" tone="blue" /><RouteCard number="02" title="COMPUTER SCIENCE" subtitle="Web apps · AI · Open source" path="/developer" tone="dark" /></motion.div>
...
function RouteCard({ number, title, subtitle, path, tone }: { number: string; title: string; subtitle: string; path: RoutePath; tone: string }) {
  return <motion.button variants={fadeUp} className={`route-card ${tone}`} onClick={() => go(path)} whileHover={{ y: -8 }} whileTap={{ scale: .985 }}><span>{number}</span><h2>{title}</h2><p>{subtitle}</p><b>VIEW ROUTE <ArrowRight /></b></motion.button>;
}
...
function DesignRoute() {
  ...
  return <div className="page design-route"><section className="route-hero design-hero"><SiteHeader light /><div className="route-hero-inner"><motion.div className="hero-copy" variants={stagger} initial="hidden" animate="show"><motion.button variants={fadeUp} className="back" onClick={() => go('/')}>BACK TO PORTFOLIO</motion.button><motion.p variants={fadeUp}>KAUNG KHANT KO · GRAPHIC DESIGN</motion.p><motion.h1 variants={fadeUp}>I MAKE IDEAS<br />FEEL <em>VISIBLE.</em></motion.h1><motion.p variants={fadeUp}>Junior Graphic Designer and Auston College student creating branding, social media, packaging, posters, and thoughtful digital experiences.</motion.p><motion.span variants={fadeUp}>CLEAR HIERARCHY · ACCESSIBLE CONTRAST · REAL DATA</motion.span></motion.div><motion.div className="selected-card" initial={{ rotate: 3, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} transition={{ duration: .7, delay: .2, ease: [0.22, 1, 0.36, 1] }}><small>SELECTED WORK</small><div>FORM<br />MEETS<br />FEELING</div><b>2024–2026</b><p>BRANDING · UI/UX · ART DIRECTION</p></motion.div><small className="scroll-label">SCROLL TO EXPLORE</small></div></section>
    <section className="section cream about-section"><div className="about-intro"><Reveal><SectionTitle index="01" eyebrow="ABOUT ME" title="DESIGNING WITH HEAD + HEART." /></Reveal><Reveal><p className="big-copy">I’m a Creative Junior Graphic Designer and student at Auston College with skills across visual design, branding, programming, social media, packaging, and poster design. I combine technology and design to create effective digital solutions.</p></Reveal></div><StaggerGrid className="stat-grid"><motion.article variants={fadeUp}><strong><CountUp value={designCerts.length} /></strong><span>DESIGN CERTIFICATES</span></motion.article><motion.article variants={fadeUp}><strong><CountUp value={2} /></strong><span>RESEARCH-LED CASE STUDIES</span></motion.article><motion.article variants={fadeUp}><strong>TOOLKIT</strong><span>FIGMA · ILLUSTRATOR<br />PHOTOSHOP · AFTER EFFECTS<br />BRANDING · UI/UX · PACKAGING</span></motion.article></StaggerGrid><p className="strengths">Strengths: creative problem solving, responsible collaboration, visual storytelling, and bridging design with code.</p></section>
    ...
    <section className="section credentials"><Reveal><SectionTitle index="05" eyebrow="DESIGN CERTIFICATES" title="TRAINED TO MAKE. CURIOUS ENOUGH TO KEEP LEARNING." /></Reveal><StaggerGrid className="certificate-grid">{designCerts.map(c => <motion.article variants={fadeUp} key={c.title} whileHover={{ y: -6 }}><small>{c.issued}</small><h3>{c.title}</h3><p>{c.issuer}</p></motion.article>)}</StaggerGrid></section><Footer label="GRAPHIC DESIGN PORTFOLIO" />
  </div>;
}
...
function DeveloperRoute() {
  const gh = useGitHub();
  const stackLanguages = gh.languages.length ? gh.languages : [...programmingLanguages];
  return <div className="page developer-route"><section className="route-hero developer-hero"><SiteHeader light /><div className="route-hero-inner"><motion.div className="hero-copy" variants={stagger} initial="hidden" animate="show"><motion.button variants={fadeUp} className="back" onClick={() => go('/')}>BACK TO PORTFOLIO</motion.button><motion.p variants={fadeUp}>KAUNG KHANT KO · COMPUTER SCIENCE + AI ENGINEERING</motion.p><motion.h1 variants={fadeUp}>BUILDING USEFUL<br />THINGS, <em>CAREFULLY.</em></motion.h1><motion.p variants={fadeUp}>Computer Science student in Yangon building practical AI applications, full-stack software, cybersecurity tools, and bilingual digital products.</motion.p><motion.span variants={fadeUp}>AI ENGINEER STATUS · ACTIVE · OPEN TO OPPORTUNITIES</motion.span></motion.div><motion.div className="code-card" initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: .7, ease: [0.22, 1, 0.36, 1] }}><small>portfolio.ts</small><pre>{`const maker = {\n  focus: ['web', 'ai',\n    'open source'],\n  status: 'shipping'\n}`}</pre><b>AVAILABLE FOR NEW PROJECTS</b></motion.div></div></section>
    <section className="section cream"><Reveal><SectionTitle index="01" eyebrow="ABOUT THE DEVELOPER" title="CODE WITH CONTEXT." /></Reveal><div className="about-grid"><p className="big-copy">Creative developer, AI product builder, and Junior Graphic Designer studying Computer Science, with experience across responsive web apps, desktop software, PWAs, APIs, and command-line tools.</p><StaggerGrid className="dev-stats"><motion.article variants={fadeUp}><strong><CountUp value={gh.publicRepos} /></strong><span>PUBLIC REPOSITORIES</span></motion.article><motion.article variants={fadeUp}><strong><CountUp value={gh.deployedCount} /></strong><span>LIVE DEPLOYMENTS</span></motion.article><motion.article variants={fadeUp}><strong><CountUp value={gh.languages.length || programmingLanguages.length} /></strong><span>LANGUAGES USED</span></motion.article><motion.article variants={fadeUp}><strong>∞</strong><span>CURIOSITY</span></motion.article></StaggerGrid></div><StaggerGrid className="developer-profile-grid"><motion.article variants={fadeUp} className="ai-status-card"><small>AI ENGINEER STATUS</small><h3>ACTIVE + BUILDING</h3><p>Developing practical AI applications and open to AI, software, and product engineering opportunities.</p></motion.article><motion.article variants={fadeUp}><small>SPOKEN LANGUAGES</small><h3>COMMUNICATION</h3><ul>{profile.languages.map(language => <li key={language.name}><b>{language.name}</b><span>{language.level}</span></li>)}</ul></motion.article><motion.article variants={fadeUp}><small>PROGRAMMING LANGUAGES</small><h3>CORE STACK</h3><div className="language-tags">{stackLanguages.map(language => <span key={language}>{language}</span>)}</div></motion.article></StaggerGrid></section>
    <section className="section forest"><Reveal><SectionTitle index="02" eyebrow="FEATURED PROJECTS" title="FROM REPO TO REAL WORLD." copy={`A few builds worth opening — ${gh.publicRepos} public repositories in total on github.com/${gh.login}.`} /></Reveal><StaggerGrid className="repo-grid">{gh.featured.map((r, i) => <motion.article key={r.name} variants={fadeUp} whileHover={{ y: -8 }} className={i === 2 ? 'wide' : ''}><span>{String(i + 1).padStart(2, '0')}</span><h3>{r.title}</h3><p>{r.description}</p><div>{r.language && <b>{r.language}</b>}{r.stars > 0 && <b><Star /> {r.stars}</b>}</div><div className="repo-links"><a href={r.url} target="_blank" rel="noreferrer">CODE <Github /></a>{r.homepage && <a href={r.homepage} target="_blank" rel="noreferrer">LIVE <ExternalLink /></a>}</div></motion.article>)}</StaggerGrid><a className="repo-all" href={`${gh.profileUrl}?tab=repositories`} target="_blank" rel="noreferrer">ALL {gh.publicRepos} REPOSITORIES ON GITHUB <ArrowRight /></a></section>
    <section className="section blue"><Reveal><SectionTitle index="03" eyebrow="SELECTED PROJECT · HACKATHON" title="A WEEKEND BUILD THAT SHIPPED." /></Reveal><motion.article className="hack-feature" whileHover={{ y: -5 }}><div className="hack-panel"><b>CURSOR AI HACKATHON MYANMAR</b><img className="hack-logo" src="/pyan-thone/logo.webp" alt="Pyan Thone" width={164} height={164} /><span>UI/UX + FRONTEND · AUGUST 2026</span></div><div><h3>PYAN THONE</h3><p>A trusted second-hand marketplace with condition evidence on every listing and verified seller trust scores—buy and sell used products safely with people nearby. Bilingual EN / မြန်မာ.</p><div className="hack-tags"><b>Leadership</b><b>Frontend</b><b>Figma</b></div><div className="hack-actions"><a className="primary" href="https://pyan-thone-sigma.vercel.app" target="_blank" rel="noreferrer">OPEN LIVE DEMO <ExternalLink /></a><button className="ghost" onClick={() => go('/case-study/cursor-ai-hackathon')}>READ CASE STUDY <ArrowRight /></button></div></div></motion.article></section>
...
function HackathonCaseStudy() {
  return <div className="page case-page">
    <section className="case-hero"><SiteHeader light /><button className="back" onClick={() => go('/developer')}><ArrowLeft /> Computer science</button>
      ...
      <aside className="case-brand"><img src="/pyan-thone/logo.webp" alt="Pyan Thone logo" width={300} height={300} /></aside>
    </section>
