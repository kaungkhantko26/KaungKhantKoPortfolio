...
type RoutePath = '/' | '/design' | '/developer' | '/case-study/austonian-hub' | '/design/su-yet-designs';

const routes: RoutePath[] = ['/', '/design', '/developer', '/case-study/austonian-hub', '/design/su-yet-designs'];
...
type SuYetPiece = { no: string; title: string; series: string; year: string; medium: string; src: string; note: string };
const suYetPieces: SuYetPiece[] = [
  { no: '01', title: 'Open Source', series: 'Tech Explained', year: '2026', medium: 'Digital poster · editorial collage', src: '/su-yet/open-source.webp', note: 'On freedom over price: study, modify, and share. A CRT-headed figure carries the code while a rusted padlock hangs open above it.' },
  { no: '02', title: 'Incognito Mode', series: 'Privacy Myths', year: '2026', medium: 'Digital poster · photo-montage', src: '/su-yet/incognito-mode.webp', note: 'Private is not anonymous. A classical bust in dark glasses and a fedora—watched by a red targeting reticle—asks what the browser actually hides.' },
  { no: '03', title: 'Networking', series: 'Community Notes', year: '2026', medium: 'Digital poster · torn-paper collage', src: '/su-yet/networking.webp', note: 'Connection turning into collaboration: many hands assembling one red arch from separate blocks—idea to project to opportunity.' },
];
...
function SuYetDesigns() {
  ...
  return <div className="page museum-page"><section className="route-hero museum-hero"><SiteHeader light /><div className="route-hero-inner"><div className="hero-copy"><button className="back" onClick={() => go('/design')}><ArrowLeft /> Graphic design</button><p>KAUNG KHANT KO · COLLECTION 01</p><h1>SU YET<br />DESIGNS.</h1><p>A curated hall of editorial poster work—technology and privacy ideas translated into bold, bilingual visual essays for the SUYET publication.</p><span>ON VIEW · 3 WORKS · YANGON · 2026</span></div><motion.div className="selected-card" initial={{ rotate: 3, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }}><small>THE COLLECTION</small><div>SU<br />YET<br />DESIGNS</div><b>2026</b><p>POSTER · EDITORIAL · COLLAGE</p></motion.div><small className="scroll-label">ENTER THE GALLERY</small></div></section>
    <section className="section cream"><Reveal><SectionTitle index="00" eyebrow="CURATORIAL NOTE" title="THREE POSTERS, ONE VOICE." copy="Each piece pairs a single technology idea with a constructed image—classical sculpture, salvaged hardware, working hands—rendered in a restrained grey palette cut by a single insistent red. English headline, Burmese body: the series is written for readers who move between both." /></Reveal></section>
    <section className="section forest museum-hall"><Reveal><SectionTitle index="01" eyebrow="MAIN GALLERY" title="ON THE WALL." /></Reveal>
      <div className="gallery-wall">{suYetPieces.map((p, i) => <motion.figure key={p.no} className="artwork" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-6%' }} transition={{ delay: i * .08, duration: .6 }}>
        <button className="frame" onClick={() => setActive(i)} aria-label={`View ${p.title} enlarged`}><img src={p.src} alt={`${p.title} — SUYET ${p.series} poster`} loading="lazy" /></button>
        <figcaption className="wall-label"><span className="wl-no">{p.no}</span><h3>{p.title}</h3><p className="wl-meta">Kaung Khant Ko · {p.year}</p><p className="wl-series">SUYET · {p.series}</p><p className="wl-medium">{p.medium}</p><p className="wl-note">{p.note}</p></figcaption>
      </motion.figure>)}</div>
    </section>
    <section className="section sage"><Reveal><SectionTitle index="02" eyebrow="ABOUT THE SERIES" title="SUYET / TECH EXPLAINED." copy="SUYET is an independent knowledge publication. This collection covers its first three explainer drops—Open Knowledge, Privacy Myths, and Community Notes—each designed to be legible at thumbnail scale in a feed and rewarding at full size on a wall." /></Reveal>
    ...
    <AnimatePresence>{piece && <motion.div className="lightbox" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setActive(null)}>
      ...
      <motion.figure className="lb-inner" key={piece.no} initial={{ scale: .96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} onClick={e => e.stopPropagation()}>
        ...
        <figcaption><span>{piece.no} · SUYET / {piece.series}</span><h3>{piece.title}</h3><p>{piece.medium} · {piece.year}</p><p className="lb-note">{piece.note}</p></figcaption>
      </motion.figure>
...
function App() {
  ...
  const page = path === '/design' ? <DesignRoute /> : path === '/design/su-yet-designs' ? <SuYetDesigns /> : path === '/developer' ? <DeveloperRoute /> : path === '/case-study/austonian-hub' ? <CaseStudy /> : <Landing />;
