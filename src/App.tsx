...
function SiteHeader({ light = false }: { light?: boolean }) {
  ...
  return <>
    <header className={`site-header ${light ? 'light' : ''}`}>
      <button className="brand" onClick={() => go('/')} aria-label="Home">KK / PORTFOLIO</button>
    ...
    <AnimatePresence>{open && <motion.div className="mobile-nav" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><button className="close" onClick={() => setOpen(false)}><X /></button>{items.map(([label, path], i) => <motion.button key={path} initial={{ x: 30, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: i * .06 }} onClick={() => { setOpen(false); go(path); }}>{label}</motion.button>)}</motion.div>}</AnimatePresence>
  </>;
}

function Footer({ label = 'PORTFOLIO / 2026' }: { label?: string }) {
  return <footer className="connect"><div><p className="eyebrow">CONNECT WITH ME</p><h2>Let’s build something useful.</h2><p>Available for design, development, and collaborative product work.</p></div><div className="contact-links"><a href={profile.linkedin} target="_blank" rel="noreferrer"><Linkedin /> LinkedIn</a><a href={profile.github} target="_blank" rel="noreferrer"><Github /> GitHub</a><a href={profile.website} target="_blank" rel="noreferrer"><ExternalLink /> Website</a><a href={`mailto:${profile.email}`}><Mail /> Email</a></div><small>{label}</small></footer>;
}
...
function DesignRoute() {
  ...
  return <div className="page design-route"><section className="route-hero design-hero"><SiteHeader light /><div className="hero-copy"><button className="back" onClick={() => go('/')}><ArrowLeft /> Portfolio</button><p>KAUNG KHANT KO · GRAPHIC DESIGN</p><h1>I MAKE IDEAS<br />FEEL VISIBLE.</h1><p>Junior Graphic Designer and Auston College student creating branding, social media, packaging, posters, and thoughtful digital experiences.</p><span>CLEAR HIERARCHY · ACCESSIBLE CONTRAST · REAL DATA</span></div><motion.div className="selected-card" initial={{ rotate: 3, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }}><small>SELECTED WORK</small><div>FORM<br />MEETS<br />FEELING</div><b>2024–2026</b><p>BRANDING · UI/UX · ART DIRECTION</p></motion.div></section>
    <section className="section cream"><Reveal><SectionTitle index="01" eyebrow="ABOUT ME" title="DESIGNING WITH HEAD + HEART." /></Reveal><div className="about-grid"><Reveal><p className="big-copy">I’m a Creative Junior Graphic Designer and Computer Science student at Auston College with skills across visual design, branding, programming, social media, packaging, and poster design.</p></Reveal><div className="stat-grid"><article><strong>03</strong><span>DESIGN CERTIFICATES</span></article><article><strong>01</strong><span>RESEARCH-LED CASE STUDY</span></article><article><strong>TOOLKIT</strong><span>FIGMA · ILLUSTRATOR · PHOTOSHOP</span></article></div></div></section>
